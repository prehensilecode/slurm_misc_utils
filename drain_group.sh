#!/bin/bash
### Drains a group of nodes. Typically, a chassis containing multiple
### nodes. For 1U servers, some group of nodes defined in /etc/genders
###    scontrol update nodename=foo1,foo2,... state=drain reason=maint
...
echo "Draining ${nodes} ..."
scontrol update nodename=${nodes} state=drain reason=maint
